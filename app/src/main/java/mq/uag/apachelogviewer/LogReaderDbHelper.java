...
public class LogReaderDbHelper extends SQLiteOpenHelper {
    ...
    {
        Cursor cursor = db.rawQuery("SELECT _id FROM remoteHost WHERE remote_host_name='"+hName+"'",null);
        if(cursor != null)
            cursor.moveToFirst();
    ...
    public List<LogEntry> get200LogEntries(int start)
    {
        ...
        String SQL= "SELECT (SELECT remote_host_name FROM remoteHost as rh WHERE rh._id = le.remote_host) as remote_host , date ," +
                ...
                "FROM logEntry as le ORDER BY date DESC LIMIT "+ start +","+(start+200);
     ...
     *
     * Exercice .1.4
     * Décrire le fonctionnement de cette fonction
     *
     */
